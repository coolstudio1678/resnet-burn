// Declare modules: resnet (copied in prev step) and imagenet (to be copied in this step)
mod imagenet;
mod resnet;
...
// Adjust path to resnet items if necessary, after `mod resnet;` it should be `resnet::...`
use resnet::{weights, ResNet};
// Adjust path to imagenet items
use imagenet::{Normalizer, CLASSES};
...
fn run_inference_on_image(
    ...
    let normalizer = Normalizer::<TchBackend<f32>>::new(device); // Use local Normalizer
    ...
    println!(
        "  Predicted: {}\n  Category Id: {}\n  Score: {:.4}\n",
        CLASSES[class_idx], // Use local CLASSES
        class_idx,
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Model is now from the local resnet module
    let model: ResNet<TchBackend<f32>> =
        resnet::ResNet::resnet18_pretrained(resnet::weights::ResNet18::ImageNet1kV1, &device)
            .map_err(|err| format!("Failed to load pre-trained weights.\nError: {err}"))?;

    println!("Model loaded successfully on device: {:?}", device);

    let samples_dir = "samples/"; // This will be relative to where the binary is run.
                                 // If running with `cargo run` from `image_classifier_app/`,
                                 // it will look for `image_classifier_app/samples/`.
